...
- **包信息查询**: 自动查询进程所属的 Debian 软件包名称及包描述信息。
...
- `dpkg`: 提供 `dpkg -S` 和 `dpkg -s` 命令，用于查询进程所属软件包信息。
...
| 用户 | 启动进程的用户 |
| CPU使用率 (%) | CPU使用百分比 |
| 内存使用量 (MB) | 实际使用的物理内存（MB） |
| 共享内存 (MB) | 共享内存大小（MB） |
| 内存使用率 (%) | 内存使用百分比 |
| 启动命令 | 进程的完整命令行 |
| 所属包名 | 进程可执行文件所属的Debian软件包名称 |
| 包描述信息 | 软件包的描述信息 |

**注意**: 如果进程的可执行文件不属于任何已安装的Debian软件包（如用户自行编译的程序、脚本等），所属包名和包描述信息字段将为空。
